...
for ($lineup_index = 0; $lineup_index < count($this->_['lineup']); $lineup_index++) {
	...
	printf("\t\t" .	'</span>
		<label for="addition_%1$u" hidden>Addition</label>
		...
		<button type="button" onclick="set_band_lineup(%2$u)">
			<img src="%3$s/plus_small.png" alt="add"/>
		</button>
		<button type="button" onclick="del_band_lineup(%1$u)"><img src="%3$s/minus_small.png" /></button>
		<button type="button" onclick="shift_band_lineup(%1$u, \'up\')">
			<img src="%3$s/arrow_up_small.png" alt="up">
		</button>
		<button type="button" onclick="shift_band_lineup(%1$u, \'down\')">
			<img src="%3$s/arrow_down_small.png" alt="down">
		</button>' . "\n\t\t</fieldset>\n\n",
	$lineup_index, $lineup_index + 1, $this->image_path,
		htmlspecialchars($this->_['lineup'][$lineup_index]['addition'], ENT_QUOTES));
}
?>
